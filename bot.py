...
CARD_NUMBER = "2202205010885801"  # Например: 4276 1234 5678 9012
ADMIN_ID = None  # Определится автоматически
    ...
    if user_id not in paid_users:
        ...
        if user_data[user_id]["count"] >= FREE_LIMIT:
            await update.message.reply_text(
                f"⚠️ Лимит исчерпан ({FREE_LIMIT} расчётов).\n\n"
                f"Для продолжения переведите {PRICE}₽ на карту:\n`{CARD_NUMBER}`\n\n"
                f"И напишите: *Оплатил*",
                parse_mode="Markdown"
            )
            return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.lower() == "оплатил":
        ...
        await update.message.reply_text("✅ Сообщил админу. Доступ откроют вручную.")
        return
...
async def approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            await context.bot.send_message(
                chat_id=target_id,
                text="✅ Доступ открыт! /start для расчёта."
            )
        except:
            pass
    except:
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👋 /start для начала.")
